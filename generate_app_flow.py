              ...
              <g id="diagramViewport" transform="matrix(1 0 0 1 0 0)">
                ...
                <g id="nodesGroup">
                  ...
                  <g id="node-screen1" class="diagram-node selected" transform="translate(950, 80)" onclick="selectScreenNode(1)">
                    ...
                    <text x="125" y="204" font-size="9.5" fill="#475569" font-weight="700" text-anchor="middle">👦 البطل عُمَر | مستكشف صغير (1/37 سورة)</text>
                  ...
                  <g id="node-screen4" class="diagram-node parent-gate" transform="translate(760, 440)" onclick="selectScreenNode(4)">
                    ...
                    <g class="diagram-btn-pill" transform="translate(15, 96)" onclick="event.stopPropagation(); triggerButtonFromDiagram('btn-s4-math', 4, null, 'VERIFY_CHALLENGE()')">
                      ...
                      <text x="115" y="16" font-size="9.5" font-weight="800" fill="#92400e" text-anchor="middle">🔢 مسألة أمان الوالد التفاعلية (Math Gate)</text>
                    </g>
            ...
            <div class="phone-screen-content" style="background: linear-gradient(180deg, #e0f2fe 0%, #f0fdf4 40%, #fefce8 100%);">
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                <div style="display: flex; align-items: center; gap: 6px;">
                  ...
                  <span style="background: #e0f2fe; color: #0369a1; font-size: 0.68rem; font-weight: 800; padding: 2px 6px; border-radius: 10px;">جزء عم: 1/37 سورة</span>
                </div>
                <button class="mock-btn subtle" style="padding: 3px 8px; font-size: 0.75rem;" onclick="openParentSettings(1)" title="بوابة الوالدين">⚙️ 🔒</button>
              </div>
          ...
          <div class="phone-bezel">
            ...
            <div class="phone-screen-content" style="background: linear-gradient(180deg, #f0fdf4 0%, #ffffff 100%);">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                ...
                <button class="mock-btn subtle" style="padding: 2px 8px; font-size: 0.75rem;" onclick="openParentSettings(2)" title="إعدادات الوالد">⚙️</button>
              </div>
            ...
            <div class="phone-screen-content" style="background: linear-gradient(180deg, #ffffff 0%, #f8fafc 100%);">
              ...
              <div id="mathGateBox" style="background: #f1f5f9; border: 1.5px solid #cbd5e1; border-radius: var(--radius-sm); padding: 6px 8px; margin-bottom: 8px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                  ...
                  <span id="math-gate-question" style="font-family: monospace; font-size: 0.8rem; font-weight: 900; color: #0f172a; background: #e2e8f0; padding: 2px 6px; border-radius: 4px;">9 + 6 = ?</span>
                </div>
                <div style="display: flex; gap: 4px; align-items: center;">
                  <input type="number" id="mathGateInput" placeholder="الناتج" style="width: 55px; padding: 3px 5px; font-size: 0.75rem; border: 1px solid #94a3b8; border-radius: 4px; text-align: center;" onkeydown="if(event.key==='Enter') verifyMathGateAndProceed(this.value)">
                  <button class="mock-btn" style="flex: 1; padding: 3px 6px; font-size: 0.68rem; background: #059669;" onclick="verifyMathGateAndProceed(document.getElementById('mathGateInput').value)">تحقق ودخول ✓</button>
              ...
              <div style="margin-top: auto; padding-bottom: 4px;">
                <button class="mock-btn" style="width: 100%; padding: 10px; background: #ca8a04; font-size: 0.85rem; font-weight: 900;" onclick="closeParentSettings()">
      ...
      1: {
        ...
        buttons: [
          ...
          },
          {
            ...
            desc: "عرض رتبة الطفل (مستكشف صغير: 1/37) وتشجيعه اليومي باللافتة الخشبية.",
            linkId: null
          }
        ]
      },
...
Header HUD:
- Top Right: "رِحْلة ⭐" logo with rank "مستكشف صغير" and "جزء عم: 1/37".
...
- Dynamic random math challenge: generateMathGateChallenge() generates dynamic arithmetic (e.g. 9 + 6 = ?) to prevent toddler accidental tampering.
Settings Controls:
...
- "إعادة تلاوة سورة الفلق 🔁" (Repeats same Surah to consolidate memorization).`
    ...
    function triggerButtonFromDiagram(btnId, sourceScreen, targetScreen, actionCode, linkId) {
      playChime('click');
      if (btnId === 'btn-s4-close') {
        targetScreen = parentModalSourceScreen;
        linkId = (parentModalSourceScreen === 1) ? 'link-s4-s1' : 'link-s4-s2';
      } else if (btnId === 'btn-s1-parent') {
        parentModalSourceScreen = 1;
      } else if (btnId === 'btn-s2-parent') {
        parentModalSourceScreen = 2;
      }
    ...
    // ==========================================
    // Dynamic Parent Navigation & Return Memory (WFK-01)
    let parentModalSourceScreen = 1; // Default to Screen 1

    function highlightLinkInDiagram(linkId) {
      document.querySelectorAll(".link-path").forEach(p => p.classList.remove("active-link"));
      if (linkId) {
        const linkEl = document.getElementById(linkId);
    ...
    function openParentSettings(sourceScreenId) {
      ...
      generateMathGateChallenge();
      navigateToScreenMock(4, 'فتح إعدادات الوالدين بأمان', 'btn-s' + parentModalSourceScreen + '-parent');
    }
    ...
    function generateMathGateChallenge() {
      ...
      const inputEl = document.getElementById('mathGateInput');
    ...
    function verifyMathGateAndProceed(userAnswer, sourceScreenId) {
      ...
      if (ans === expectedMathAnswer) {
        ...
        showToast('🔓 تم التحقق من أمان الوالد بنجاح!');
        if (sourceScreenId) {
          openParentSettings(sourceScreenId);
        }
        return true;
      } else {
        playChime('click');
        ...
        showToast('⚠️ إجابة غير صحيحة، تم توليد مسألة جديدة للأمان');
        generateMathGateChallenge();
        return false;
      }
    }
    ...
    let currentVerseRepeatCount = 0;
    let isAutoPlayEnabled = true;
    let isVerseTransitionLocked = false;
    const VERSE_ADVANCE_COOLDOWN_MS = 800;
    ...
    function simulateParentInvisibleTap() {
      ...
      if (!isSheikhTurn) {
        ...
        if (badge) {
          badge.innerHTML = `✨ دورك يا عمر! ردد الآية لبابا وماما في الغرفة (ترديد ${currentVerseRepeatCount + 1} من ${currentRepetitions})`;
        ...
        showToast(`🌟 وقفة الترديد: الطفل يقرأ لوالديه (${currentVerseRepeatCount + 1}/${currentRepetitions})`);
      } else {
        // Sheikh recitation state: evaluate repetition count against currentRepetitions (LOG-03)
        currentVerseRepeatCount++;
        if (currentVerseRepeatCount >= currentRepetitions) {
          currentVerseRepeatCount = 0;
          playTone(523.25, 0.3, 'sine', 0.2);
          showToast(`🎉 أحسنت يا بطل! أتممت التكرار (${currentRepetitions} مرات)`);
          advanceToNextVerse();
        } else {
          ...
          if (badge) {
            badge.innerHTML = `🎙️ الشيخ المنشاوي يعيد الآية (${currentVerseRepeatCount + 1} من ${currentRepetitions})...`;
            badge.style.color = "#065f46";
            badge.style.fontWeight = "700";
          }
          if (sub) sub.textContent = "👆 اضغط هنا لمحاكاة لمسة الوالد الخفية في أي مكان";
          showToast(`🔁 إعادة التلاوة: تكرار ${currentVerseRepeatCount + 1} من ${currentRepetitions}`);
        }
      }
    }
